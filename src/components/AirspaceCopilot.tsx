  ...
  if (!open) {
    return (
      <button
        ...
        onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(168,85,247,0.5)'; e.currentTarget.style.boxShadow = '0 0 16px rgba(168,85,247,0.25)'; }}
          ...
          >
            <div
              className={`max-w-[85%] rounded-xl px-3 py-2 text-xs leading-relaxed ${
                msg.role === "user"
                  ? "bg-purple-600/20 text-purple-100 rounded-br-sm"
                  : "rounded-bl-sm"
              }`}
          ...
          <div className="flex flex-wrap gap-1.5">
            {suggestedFollowups.map((q, i) => (
              <button
                ...
                className="text-[10px] text-purple-300 bg-purple-900/20 hover:bg-purple-900/40 border border-purple-800/30 px-2 py-1 rounded-full transition-colors"
              >
                {q}
              </button>
            ))}
      ...
      <div className="px-3 py-2" style={{ borderTop: '1px solid var(--border-default)', background: 'var(--surface-2)' }}>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-2 rounded-lg bg-purple-600/20 hover:bg-purple-600/30 text-purple-400 disabled:opacity-30 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
