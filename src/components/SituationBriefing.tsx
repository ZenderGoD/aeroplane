...
export default function SituationBriefing({
  ...
  return (
    <div className="mx-5 mb-3">
      <div className="rounded-xl border border-purple-800/40 bg-gradient-to-br from-purple-950/30 to-gray-950/80 overflow-hidden">
        {/* Header */}
        <button
          onClick={() => setExpanded((v) => !v)}
          className="w-full flex items-center gap-2 px-4 py-2.5 hover:bg-purple-900/20 transition-colors"
        >
          <svg className="w-4 h-4 text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
          </svg>
          <span className="text-sm font-semibold text-purple-300">AI Situation Briefing</span>
          {briefing && (
          ...
          <div className="px-4 pb-3">
            {!briefing && !isLoading && !error && (
              <button
                ...
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-purple-600/20 hover:bg-purple-600/30 border border-purple-700/30 text-purple-300 text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
              >
            ...
            {isLoading && (
              <div className="flex items-center justify-center gap-2 py-6">
                ...
                <span className="text-xs text-purple-400">Analyzing airspace...</span>
              </div>
            )}

            {error && (
              <div className="text-center py-4">
                <p className="text-xs text-red-400">Failed to generate briefing</p>
            ...
            {briefing && !isLoading && (
              <div className="space-y-3">
                {/* Summary */}
                <div>
                  <div className="text-[10px] font-bold text-purple-400 uppercase tracking-wider mb-1">Summary</div>
                ...
                {isStale && remaining > 0 && (
                  <button
                    onClick={generate}
                    className="w-full text-[10px] text-purple-400 hover:text-purple-300 py-1 transition-colors"
                  >
