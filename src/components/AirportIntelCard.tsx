...
export default function AirportIntelCard({
  ...
  if (!analysis && !isLoading && !error) {
    return (
      <button
        onClick={analyze}
        className="w-full mt-2 flex items-center justify-center gap-1.5 py-1.5 rounded-md bg-gray-700/10 hover:bg-gray-700/20 border border-gray-700/30 text-gray-300 text-xs font-medium transition-colors"
      >
  ...
  if (isLoading) {
    return (
      <div className="mt-2 flex items-center justify-center gap-2 py-3">
        ...
        <span className="text-xs text-gray-400">Analyzing {airportIcao}...</span>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2 rounded-lg border border-gray-700/30 bg-gray-900/15 px-3 py-2.5 space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-xs font-bold text-gray-300 uppercase tracking-wider">AI Intelligence</span>
        <span className={`text-[11px] font-bold uppercase px-1.5 py-0.5 rounded ${badge.bg} ${badge.text}`}>
          {riskLevel}
        </span>
      </div>
      ...
      {analysis.situation && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase">Situation</div>
          <p className="text-xs text-gray-300 leading-relaxed">{analysis.situation}</p>
        </div>
      )}

      {analysis.comparison && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase">vs Baseline</div>
          <p className="text-xs text-gray-300 leading-relaxed">{analysis.comparison}</p>
        </div>
      )}

      {analysis.weatherImpact && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase">Weather Impact</div>
          <p className="text-xs text-gray-300 leading-relaxed">{analysis.weatherImpact}</p>
        </div>
      )}

      {analysis.outlook && (
        <div>
          <div className="text-[11px] font-semibold text-gray-500 uppercase">Outlook</div>
          <p className="text-xs text-gray-400/80 leading-relaxed">{analysis.outlook}</p>
        </div>
      )}
    </div>
  );
