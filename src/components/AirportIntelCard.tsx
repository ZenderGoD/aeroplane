...
const RISK_BADGE: Record<string, { bg: string; text: string }> = {
  normal: { bg: "bg-green-900/40", text: "text-green-400" },
  elevated: { bg: "bg-amber-900/40", text: "text-amber-400" },
...
export default function AirportIntelCard({
  ...
  if (!analysis && !isLoading && !error) {
    return (
      <button
        onClick={analyze}
        className="w-full mt-2 flex items-center justify-center gap-1.5 py-1.5 rounded-md bg-cyan-600/10 hover:bg-cyan-600/20 border border-cyan-800/30 text-cyan-400 text-[10px] font-medium transition-colors"
      >
  ...
  if (isLoading) {
    return (
      <div className="mt-2 flex items-center justify-center gap-2 py-3">
        ...
        <span className="text-[10px] text-cyan-400">Analyzing {airportIcao}...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="mt-2 text-center py-2">
        <p className="text-[10px] text-red-400">Analysis failed</p>
        <button onClick={analyze} className="text-[10px] text-cyan-400 hover:underline">Retry</button>
      </div>
    );
  }
  ...
  return (
    <div className="mt-2 rounded-lg border border-cyan-800/30 bg-cyan-950/15 px-3 py-2.5 space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-bold text-cyan-400 uppercase tracking-wider">AI Intelligence</span>
      ...
      {analysis.outlook && (
        <div>
          ...
          <p className="text-[10px] text-amber-400/80 leading-relaxed">{analysis.outlook}</p>
        </div>
      )}
    </div>
  );
